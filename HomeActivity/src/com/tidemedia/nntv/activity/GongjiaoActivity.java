 ...
 */
public class GongjiaoActivity extends BaseActivity implements
		OnGetPoiSearchResultListener, BaiduMap.OnMapClickListener,
		OnGetRoutePlanResultListener {
	private PoiSearch mSearch = null; // 搜索模块，也可去掉地图模块独立使用
	private ListView mListView;
	private GongjiaozhandianlistAdapter mGongjiaolistAdapter;
	...
	public void onGetPoiResult(PoiResult result) {
		...
		if (result == null || result.error != SearchResult.ERRORNO.NO_ERROR) {
			Toast.makeText(GongjiaoActivity.this, "抱歉，未找到结果", Toast.LENGTH_LONG)
					.show();
			return;
		}
		...
		View view = LayoutInflater.from(this).inflate(
				R.layout.layout_gongjiaoxianlu_list_item, null, false);
		mListView.addHeaderView(view);
		mGongjiaolistAdapter = new GongjiaozhandianlistAdapter(
				getApplicationContext(), poilist, mListView);
		...
		mListView.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view,
				...
				List<String> list = mGongjiaolistAdapter.getList();
				String newsModel = list.get(position - 1);
				if (null != newsModel) {
					Intent intent = new Intent(getApplicationContext(),
