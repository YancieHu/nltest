...
public class DemoFragmentAdapter extends PagerAdapter {
    ...
    public DemoFragmentAdapter(FragmentManager fm, boolean pIfForImage, List<Category> list) {
        mFragmentManager = fm;
        mIfForImage = pIfForImage;
        if(!pIfForImage){
        	for (Category category : list) {
            	mFragmentList.add(new ListViewFragment(category.getCat_id()));
    		}
        }else{
        	for (Category category : list) {
            	mFragmentList.add(new ImageFragment(category.getCat_id()));
    		}
        }
    }
    @Override
    ...
    protected  String getTag(int position){
    	if(mIfForImage){
    		 return ((ImageFragment)mFragmentList.get(position)).getText();
    	}else{
    		 return ((ListViewFragment)mFragmentList.get(position)).getText();
    	}
    }
