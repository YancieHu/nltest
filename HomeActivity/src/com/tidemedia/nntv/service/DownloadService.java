...
public class DownloadService extends Service {
	...
	private final String fileName = "南宁头条"; // 显示的文件名称
	private final String apkName = "nanningtoutiao.apk";// 下载文件的名称
	...
	private void createNotification(int notifyId) {
		switch (notifyId) {
		case NOTIFY_DOW_ID:
			int icon = R.drawable.ic_nntv_launcher;
			...
			break;
		case NOTIFY_OK_ID:
			int icon2 = R.drawable.ic_nntv_launcher;
			...
			PendingIntent contentInten2 = PendingIntent.getActivity(mContext,
					0, intent, 0);
			mNotification.setLatestEventInfo(mContext, fileName, "已下载完毕",
					contentInten2);
			stopSelf();// 停掉服务自身
	...
	private void startDownload() {
		...
		try {
			...
			String downloadPath = TvApplication.getInstance().getDirPath("download");
			if (Environment.getExternalStorageState().equals(
					android.os.Environment.MEDIA_MOUNTED)) {// 判断是否有sd卡
				LogUtil.d("sd卡路径", downloadPath + "/" + apkName);
				// 创建目录
				//File apkDir = new File(TuJiaApplication.mSdcardDataDir + TuJiaApplication.mDownloadPath);
//				if (!apkDir.exists()) {
//					apkDir.mkdir();
//				}
